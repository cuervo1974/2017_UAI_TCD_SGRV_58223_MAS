Public Class FormPresupuesto
    ...
    Sub New(ByRef pObj As Estructura.Presupuesto)
        InitializeComponent()
        Me._Obj = pObj
    End Sub
    ...
    Private Sub OK_Button_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles OK_Button.Click
        Try
            BLL.PresupuestoLogica.Validar(Me._Obj)
    ...
    Private Sub RellenarDataGrid(pListado As List(Of Estructura.PresupuestoDetalle))
    ...
    Private Sub ButtonAgregarProducto_Click(sender As Object, e As EventArgs) Handles ButtonAgregar.Click
        Try
            ...
            End If
            If Not IsNothing(Me._Obj.PresupuestoDetalle.Find(Function(x) x.Producto.Id = Producto.Id)) Then Throw New Exception("Ya existe ese Producto en este Presupuesto.")
            Dim PresupuestoDetalle = New Estructura.PresupuestoDetalle(Producto.Precio, Cantidad, Descuento)
            PresupuestoDetalle.Producto = Producto
            Me._Obj.PresupuestoDetalle.Add(PresupuestoDetalle)
            Me.RellenarDataGrid(Me._Obj.PresupuestoDetalle)
    ...
    Private Sub DataGridProductos_CellContentClick(sender As Object, e As DataGridViewCellEventArgs) Handles DataGridProductos.CellContentClick
        ...
        Dim PresupuestoDetalle As Estructura.PresupuestoDetalle = Me.DataGridProductos.Rows(e.RowIndex).DataBoundItem
        Select Case Me.DataGridProductos.Columns(e.ColumnIndex).Name
            Case "Eliminar"
                Me._Obj.PresupuestoDetalle.Remove(PresupuestoDetalle)
                Me.RellenarDataGrid(Me._Obj.PresupuestoDetalle)
                Me.LabelTotal.Text = String.Format("$ {0}", Me._Obj.Total)
        End Select
    End Sub
